...
- **Login/register 500 with "password authentication failed for user postgres"**: The username in `DATABASE_URL` must be **postgres.&lt;project-ref&gt;** (e.g. `postgres.qztsitnpzkdhyysmjapg`), not plain `postgres`. Use the full Supavisor Transaction URI from the dashboard (Connect → Transaction).
